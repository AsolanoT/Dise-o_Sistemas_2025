import {
  ...
  import { documentText, calendar, cash, arrowBack, refresh } from 'ionicons/icons';
...
import { getCurrentUser } from '../../../../services/role.service';
  ...
  export function ViewFacturasContribuyente() {
    ...
    const formatEstado = (estado: string) => {
      ...
      const estadoInfo = estados[estado.toLowerCase()] || { color: 'primary', text: estado };
      return (
        <IonBadge color={estadoInfo.color}>{estadoInfo.text}</IonBadge>
      );
    };
        ...
        <IonContent>
          ...
          {isLoading ? (
            ...
            <div className="ion-text-center ion-padding">
              <IonText color="medium">No se encontraron facturas a su nombre</IonText>
            </div>
          ) : (
                        ...
                        <IonCol>
                          ...
                          <IonCardSubtitle>
                            <IonText color="medium">
                              Periodo: {factura.periodo} • {formatEstado(factura.estado)}
                            </IonText>
                          </IonCardSubtitle>
                        </IonCol>
                      </IonRow>
                      ...
                      <IonRow>
                        <IonCol size="6">
                          <IonChip color="light">
                            ...
                            <IonLabel>Emisión: {formatDate(factura.fechaEmision)}</IonLabel>
                          </IonChip>
                        </IonCol>
                        <IonCol size="6">
                          <IonChip color="light">
                            <IonIcon icon={calendar} color="primary" />
                            <IonLabel>Vencimiento: {formatDate(factura.fechaVencimiento)}</IonLabel>
                          </IonChip>
                        </IonCol>
                      </IonRow>
                      ...
                      <IonRow>
                        <IonCol size="6">
                          <IonChip color="light">
                            ...
                            <IonLabel>Base: {formatCurrency(factura.baseCalculo)}</IonLabel>
                          </IonChip>
                        </IonCol>
                        <IonCol size="6">
                          <IonChip color="light">
                            <IonIcon icon={cash} color="primary" />
                            <IonLabel>Total: {formatCurrency(factura.valorEstimado)}</IonLabel>
                          </IonChip>
                        </IonCol>
                      </IonRow>
                    </IonGrid>
